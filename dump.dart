...
class _GhanaTownGeneratorState extends State<GhanaTownGenerator> {
  ...
  Future<void> _initializeDatabase() async {
    ...
    if (count == 0) {
      await loadTownNamesFromCSV(db);
    }

    // Load town names from the database
  ...
  Future<void> loadTownNamesFromCSV(Database db) async {
    final bytes = await rootBundle.load('assets/ghanaian_towns.csv'); // Replace with your CSV path
    final string = utf8.decode(bytes.buffer.asUint8List());
    final List<List<dynamic>> csvData = const CsvCodec().decode(string);

    await db.transaction((txn) async {
      for (final row in csvData) {
        final townName = row[0] as String; // Assuming 'FULL_NAME' is the first column
  ...
  void generateRandomTown() {
    ...
    setState(() {
      generatedTown = ghanaianTowns[randomIndex];
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Ghana Town Generator'),
      ),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
